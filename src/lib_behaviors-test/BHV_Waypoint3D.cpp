...
void BHV_Waypoint3D::onSetParamComplete()
{
...
bool BHV_Waypoint3D::setParam(string param, string param_val) 
{
...
void BHV_Waypoint3D::onIdleToRunState() 
{
  if(!updateInfoIn()) 
    return;
  m_odo_leg_disq = true;
...
void BHV_Waypoint3D::onRunToIdleState() 
{
  postErasables();
  if(m_reset_on_idle)
...
BehaviorReport BHV_Waypoint3D::onRunState(string input) 
{
...
void BHV_Waypoint3D::onIdleState() 
{
  if(!updateInfoIn()) 
    return;
}
...
bool BHV_Waypoint3D::setNextWaypoint()
{
  m_waypt_hit = false;
...
void BHV_Waypoint3D::postStatusReport()
{
  int    current_waypt = m_waypoint_engine.getCurrIndex();
...
void BHV_Waypoint3D::postViewableSegList()
{
...
void BHV_Waypoint3D::postErasables()
{
  postMessage("VIEW_POINT", m_trackpt.get_spec_inactive(), "trk");
  postMessage("VIEW_POINT", m_nextpt.get_spec_inactive(), "wpt");
...
void BHV_Waypoint3D::checkLeadConditions()
{
  if(!m_info_buffer) 
    return;
...
void BHV_Waypoint3D::updateOdoDistance()
{
  if(!m_odo_virgin) {
...
void BHV_Waypoint3D::markOdoLeg()
{
...
void BHV_Waypoint3D::postConfigStatus()
{
  string str = "type=BHV_Waypoint3D,name=" + m_descriptor;
...
string BHV_Waypoint3D::expandMacros(string sdata)
{
  // =======================================================
...
string BHV_Waypoint3D::getReverseStr() const
{
